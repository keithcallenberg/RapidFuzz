...
cdef extern from "cpp_process_eofm.hpp":
    object eofm_two_lists_impl[T](    const RF_Kwargs*, RF_Scorer*,
        const vector[RF_StringWrapper]&, const vector[RF_StringWrapper]&, int, int, T) except +
    void set_score(PyArrayObject*, int, npy_intp, double)
...
cdef eofm_two_lists(queries, choices, RF_Scorer* scorer, const RF_ScorerFlags* scorer_flags, processor, score_cutoff, dtype, int c_workers, const RF_Kwargs* kwargs):
...
cdef eofm_py(queries, choices, scorer, processor, score_cutoff, dtype, workers, dict kwargs):
    proc_queries = preprocess_py(queries, processor)
    proc_choices = preprocess_py(choices, processor)

    cdef npy_intp[1] dims = [<npy_intp>proc_queries.size()]

    c_dtype = dtype_to_type_num_f64(dtype)
    #matrix = PyArray_SimpleNew(2, dims, c_dtype)
    array = PyArray_SimpleNew(1, dims, c_dtype)
    ...
    #         set_score(<PyArrayObject*>array, c_dtype, i, score)

    return array

# todo link to c api docs
def extractOneForMany(queries, choices, *, scorer=ratio, processor=None, score_cutoff=None, dtype=None, workers=1, **kwargs):
    """
    ...
    **kwargs : Any, optional
    ...
    -------
    ndarray
        Returns an array of dtype with the argmax of the distance/similarity.
    """
    ...
    if scorer_context:
        if dereference(scorer_context).version == 1:
            ...
            return eofm_two_lists(
                queries, choices, scorer_context, &scorer_flags, processor,
                score_cutoff, dtype, workers, &kwargs_context.kwargs)

    return eofm_py(queries, choices, scorer, processor, score_cutoff, dtype, workers, kwargs)
